...
def MyEnlarge(x0, y0, width, height, x1, y1, times, mean_fpr, mean_tpr, thickness=1, color = 'blue'):
    def MyFrame(x0, y0, width, height):
        ...
        x1 = np.linspace(x0, x0, num=50)  # 生成列的横坐标，横坐标都是x0，纵坐标变化
        y1 = np.linspace(y0, y0, num=50)
        xk = np.linspace(x0, x0 + width, num=50)
        yk = np.linspace(y0, y0 + height, num=50)
    ...
    # 连接两个虚线框
    xp = np.linspace(x0 + width, x1, num=50)
    yp = np.linspace(y0, y1 + height2, num=50)
...
while counter0 < 5:
    ...
    #                   label='fold %d (AUC = %0.4f)' % (i, roc_auc))
    MyEnlarge(0, 0.75, 0.25, 0.25, 0.5, 0, 2, mean_fpr, tprs[i], 1.5, colorlist[i])
...
storFile(MyNew, 'LOW五折的表（由保存值生成）.csv')
print(MyNew)
...
plt.plot(mean_fpr, mean_tpr, color='black',
         ...
         lw=2, alpha=0.8)
MyEnlarge(0, 0.75, 0.25, 0.25, 0.5, 0, 2, mean_fpr, mean_tpr, 2, colorlist[5])
...
# # 保存图片
plt.savefig('L_ROC-5fold.svg')
plt.savefig('L_ROC-5fold.tif')
plt.show()
