...
if __name__ == '__main__':
    ...
    relationship = []
    read_csv(relationship, 'phage host pairs.csv')
    random.shuffle(relationship)    # random shuffle
    relationship_train = relationship[0: int(0.7 * len(relationship))]
    store_csv(relationship_train, '../分类器对比实验/PositiveSample_Train_all_phagehost9885.csv')
    relationship_validation = relationship[int(0.7 * len(relationship)):int(0.8 * len(relationship))]
    store_csv(relationship_validation, '../分类器对比实验/PositiveSample_Validation_all_phagehost9885.csv')
    relationship_test = relationship[int(0.8 * len(relationship)):]
    store_csv(relationship_test, '../分类器对比实验/PositiveSample_Test_all_phagehost9885.csv')
    ...
    relationship_matrix.to_csv('./line0/phagehost_Relationship_Matrix.csv')

    store_csv(negative_sample, '../分类器对比实验/NegativeSample_all_phagehost9885.csv')
    negative_sample_train = negative_sample[0: int(0.7 * len(negative_sample))]
    store_csv(negative_sample_train, '../分类器对比实验/NegativeSample_Train_all_phagehost9885.csv')
    negative_sample_validation = negative_sample[int(0.7 * len(negative_sample)):int(0.8 * len(negative_sample))]
    store_csv(negative_sample_validation, '../分类器对比实验/NegativeSample_Validation_all_phagehost9885.csv')
    negative_sample_test = negative_sample[int(0.8 * len(negative_sample)):]
    store_csv(negative_sample_test, '../分类器对比实验/NegativeSample_Test_all_phagehost9885.csv')
