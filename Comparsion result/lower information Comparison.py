...
if __name__ == '__main__':
    ...
    while CounterT < 5:
        ...
        x1 = Conv2D(16, activation='relu', kernel_size=(4, 64),  name='conv1')(input1)
        x1 = MaxPooling2D(pool_size=(1, 1), name='pool1')(x1)
        x1 = Flatten()(x1)
        x1 = Dense(32,  activation='relu', activity_regularizer=regularizers.l2(0.0009))(x1)

        # ——输入2——
        input2 = Input(shape=(len(x_train_2_Attribute[0]), len(x_train_2_Attribute[0][0]), 1), name='input2')
        x2 = Conv2D(16, kernel_size=(4, 64),  name='conv2')(input2)
        ...
        x3 = Dense(32, activation='relu', activity_regularizer=regularizers.l2(0.001))(input3)
        ...
        hidden = Dropout(rate=0.1)(hidden)
        hidden = Dense(128, activation='relu', name='hidden4', activity_regularizer=regularizers.l2(0.001))(hidden)
        hidden = Dropout(rate=0.1)(hidden)
        ...
        while counter < len(ModelTestOutput):
            ...
            counter = counter + 1
        LabelPredictionProbName = 'deepwalkRealAndPredictionProbA+B' + str(CounterT) + '.csv'
        StorFile(LabelPredictionProb, LabelPredictionProbName)
        LabelPredictionName = 'deepwalkRealAndPredictionA+B' + str(CounterT) + '.csv'
